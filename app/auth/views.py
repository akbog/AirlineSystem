...
from ..models import Customer, User, Booking_agent, Airline_staff, Airport, Role, Permission
...
def customerlogin():
    ...
    if form.validate_on_submit():
        ...
        #rendered again
    return render_template('auth/login.html', form = form, permissions = Permission)

@auth.route('/agentlogin', methods = ['GET', 'POST'])
def agentlogin():
    form = AgentLoginForm()
    if form.validate_on_submit():
        agent = Booking_agent.query.filter_by(booking_agent_id = form.booking_agent_id.data, email = form.email.data).first()
    ...
    return render_template('auth/agentlogin.html', form = form, permissions = Permission)
    ...
    if form.validate_on_submit():
        ...
        if partner is not None:
            ...
            if user.verify_password(form.password.data):
                ...
                return redirect(request.args.get('next') or url_for('main.index'))
        flash('Invalid Login Credentials.')
    return render_template('auth/partnerlogin.html', form = form, permissions = Permission)
...
def register_customer():
    ...
    airports = Airport.query.with_entities(Airport.country).distinct().order_by(Airport.country.asc())
    form.passport_country.choices = [(i.country,i.country) for i in airports]
    if form.validate_on_submit():
    ...
    return render_template('auth/register.html', form = form, permissions = Permission)
...
def unconfirmed():
    if current_user.is_anonymous or current_user.confirmed:
        return redirect(url_for('main.index'))
    return render_template('auth/unconfirmed.html', permissions = Permission)
